...
*/
export function formatInput(arr){
...
and returns the list of keywords generated by the NLP keyword extraction algorithm
*/
export async function extractKeywords(inputText){
  let extractedKeywords;
  const { PythonShell } = require('python-shell');
  await PythonShell.run('./app/keyword_extractor.py', {mode: 'json', pythonPath: 'python', args: [inputText]}).then(msg => {
    extractedKeywords = msg[0];
  });
...
*/
export async function generateCache(cacheCollection, postCollection, collectionDate){
  ...
  topPhrases = await extractKeywords(formatInput(arr));
