...
export default async function Mongo() {
  ...
  try {
    ...
    const cursor = collection
      .find({ publishedAt: { $regex: "2022-09" }, viewsCount: { $gt: 100 } })
      .sort({ viewsCount: -1 });
    ...
    results = results
      .map((post) => ({
        ...
        score:
          post.totalViews +
          2 * (post.totalViews - post.uniqueViews) +
          50 * post.totalComments +
          5 * post.totalLikes -
          (30 * (new Date() - new Date(post.publishedAt))) /
            (1000 * 60 * 60 * 24),
      }))
        ...
        {results
          ...
          .map((res, i) => (
            <div key={i}>
              ...
              <p className="m-3">
                Date published: {res.publishedAt.substr(0, 10)}
              </p>
            </div>
          ))}
      </div>
