...
export default function StatsGraph(props){
  ...
  const [ currentLabels, setLabels ] = useState(generateLabels(props.startDate, currentInterval));
  ...
  if(currentDayIndex < 0){ // if the currentDayIndex is less than 0 (outside of the scope of that user's activity), display no data
    postsData = [];
    commentsData = [];
    postViewsData = [];
    unansweredQuestionsData = [];
    topPostsData = [];
  }else{ // otherwise, display data as normal
    postsData = props.data.slice(currentDayIndex, currentDayIndex + currentInterval).map((obj) => obj.numPosts);
    commentsData = props.data.slice(currentDayIndex, currentDayIndex + currentInterval).map((obj) => obj.numComments);
    postViewsData = props.data.slice(currentDayIndex, currentDayIndex + currentInterval).map((obj) => obj.numPostViews);
    unansweredQuestionsData = props.data.slice(currentDayIndex, currentDayIndex + currentInterval).map((obj) => obj.numUnansweredQuestions);
    topPostsData = props.data.slice(currentDayIndex, currentDayIndex + currentInterval).map((obj) => obj.numTopPosts);
  }
  ...
  return <div style={{backgroundImage: 'linear-gradient(rgba(0, 224, 255, 0.30), rgba(240, 56, 255, 0.30))', border:'2px', borderStyle:'solid', borderRadius: '10px', padding: '20px', margin: '20px'}}>
    <Bar width={1000} height={500} options={options} data={graphData}/>
    {showIntervalButtons ? <div style={{display:'flex', justifyContent:'space-between', height:'50px'}}>
      <Button onClick={function(){
        // go to previous time interval, generate new graph labels
        setCurrentDayIndex(currentDayIndex - currentInterval);
        setLabels(generateLabels(new Date(new Date(currentLabels[0]).getTime() - (currentInterval * 24 * 60 * 60 * 1000)), currentInterval));
      }} style={{backgroundColor:'rgba(255, 255, 255, 0.5)', marginTop:'10px'}}
      ><ChevronLeftIcon/></Button>
      <div style={{marginTop:'10px'}}>{(currentInterval === 7) ? 'Week of ' + currentLabels[0].substring(5, 10) : (currentInterval === 30) ? months[new Date(currentLabels[0].substring(0, 10)).getMonth()] + ' - ' + months[new Date(currentLabels[currentInterval - 1].substring(0, 10)).getMonth()] : (currentInterval === 99) ? 'All Time' : ''}</div>
      <Button onClick={function(){
    ...
    </div> : <div style={{display:'flex', justifyContent:'center', alignItems:'center', height:'50px'}}><div>All Time</div></div>}
...
function generateLabels(startingDate, interval){
  ...
  for(let i = 0; i < interval; i++){
    newLabels.push(new Date(startingDate.getTime() + i * (24 * 60 * 60 * 1000)).toISOString().substring(0, 10));
  }
