...
afterAll(async () => { //runs after all tests are finished, closes connection
  await client.close();
});

describe('formatInput', () =>  {//private posts contain the string "zzz", these posts should be removed
...
describe('extractKeywords', () => { //testing extracting keywords with machine learning
  ...
  it('handles Python script error', async () => { //we create a python shell to run the keyword extractor, this tests if it throws an error
    const mockRun = jest.spyOn(PythonShell, 'run').mockRejectedValue(new Error('simulated Python error')) //mock a Python shell error
...
describe('generateCache', () => { //testing caching which is used all features, not just Trending Topics
  ...
  it('calls with incorrect date format are not added to cache', async () => { //check that incorrect date format doesn't add to a cache
